...
class ofApp : public ofBaseApp {
	...
	public:
		...
		// comsil final project;
		void darkmaze(); // 다른 플래그 변수를 0으로 초기화, 메모리 해제 후 다시 파일을 읽어들린 후 isdarkmaze를 1로 바꿈
		void darkmazedraw(); // 암전 미로 탈출 게임 진행을 화면에 나타냄
		void findexit(); // 사용자가 미로 탈출에 성공하면 완료 메시지 출력
		int isdarkmaze; // 암전 미로 탈출 게임이 실행되면 1
		int user_x; // 사용자의 x좌표
		int user_y; // 사용자의 y좌표
		int gameclear; // 사용자가 출구에 다다르면 1
		int hint1; // 힌트 레벨 1이 적용되면 1
		int hint2; // 힌트 레벨 2가 적용되면 1
		int hint3; // 힌트 레벨 3이 적용되면 1
};
