...
void createRankList(){
	...
	fscanf(rank, "%s %d", head->name, &head->score);
	head->link = NULL;
	for (i = 1; i < num; i++){
		int s;
		char n[NAMELEN];
...
void rank(){
	...
	switch (option){
		...
		case '2':
		{
			...
			printw("----------------------------------\n");
			while(node){
				if (!strcmp(n, node->name)){
					printw(" %-19s| %-12d\n", node->name, node->score);
					if (!search_flag) search_flag++;
				}
				node = node->link;
			}
			if (!search_flag){
		...
		case '3':
		{
			...
			FILE *delrank = fopen("rank.txt", "r");
			int initnum;
			fscanf(delrank, "%d", &initnum);
			initnum--;
			fclose(delrank);
			...
			FILE *newrank = fopen("rank.txt", "w");
			fprintf(newrank, "%d\n", initnum);
			Node *nnode = head;
			while(nnode){
				fprintf(newrank, "%s %d\n", nnode->name, nnode->score);
				nnode = nnode->link;
			}
			fclose(newrank);
...
void newRank(int score){
	...
	Node *currnode = head;
	Node *prevnode;
	if (currnode->score < newnode->score){
